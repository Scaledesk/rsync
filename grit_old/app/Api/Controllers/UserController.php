...
class UserController extends Controller
{
    ...
    public function profileDetails($profileId){
        ...
        $item = $this->model()->findOrFail($profileId);
        return $this->respondWithItem($item);
   }
    ...
    public function updateProfile(){
        ...
        //echo "<pre>";print_r($data['child_services']);exit();
        
        if(isset($data['child_services']))
        {
            $services= $data['child_services'];
            //echo "<pre>";print_r($services[0]);exit();
           $user->childServices()->sync($services);
           //$user->parentServices()->sync($services);
    ...
    public function getUserNotifications(){
        ...
        return Notifynder::getAll(Authorizer::getResourceOwnerId());
    }
    
    // public function getUserNewNotifications(){
    //    
    //    $paginatedBool = (Input::has('paginationBool') && Input::get('paginationBool') == 1)?1:0;
    //    return Notifynder::where('category_id',$categoryId)->getNotRead(Authorizer::getResourceOwnerId(),5,$paginatedBool);
    //}
    ...
    public function removeExpert($id){
        ...
        Mail::send('email.removeExpert', ['reason' => $data['reason'], 'name'=>$user->name], function ($message) use ($user) {
            $message->to($user->email, $user->name)
                    ->from('noreply@gritwings.com')
                ->subject('Your Expert Account is Removed');
        });
        return $this->respondWithArray([
            'status_code'=>200
        ]);
